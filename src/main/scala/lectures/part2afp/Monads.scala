  ...
  /*
    Monads are a kind of types which have some fundamental ops.
      ...
      trait MonadTemplate[A] {
        def unit(value: A): MonadTemplate[A]                  <---------------- also called apply
    ...
    List(x).flatMap(f) =                                        list.flatMap(x => List(x)) =
    f(x) ++ Nil.flatMap(f) =                                    list
    f(x)
    ...
    Left identity:                                              Right identity:
    Option(x).flatMap(f) = f(x)                                 opt.flatMap(x => Option(x)) = opt
    Some(x).flatMap(f) = f(x)                                   Some(v).flatMap(x => Option(x)) =
                                                                Option(v) =
                                                                Some(v)
    Associativity:
